...
{
public:
    explicit TestBackend(bool nativeLoops)
        : _nativeLoops(nativeLoops)
    {
    }

    void setSource(const QString &filesystemPath) override { source = filesystemPath; }
...
private slots:
    ...
    void resolve_qrcExtractsToCache()
    {
        const auto path = NotificationSoundPlayer::resolveToFilesystemPath(
            QStringLiteral("qrc:///client/theme/call-notification.wav"));
