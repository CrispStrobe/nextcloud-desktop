...
public:
    ...
    {
        _filesystemPath = filesystemPath.toUtf8();
    }

    void play(int /*loops*/) override
    {
        if (!_context || _filesystemPath.isEmpty()) {
            signalFinishedAsync();
            return;
        }
        ...
        ca_proplist_sets(props, CA_PROP_MEDIA_FILENAME, _filesystemPath.constData());
